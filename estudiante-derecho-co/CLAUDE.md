...
2. Si ese archivo no existe o todavía tiene marcadores [PENDIENTE], DETENERSE antes
   ...
   /estudiante-derecho-co:entrevista-inicial — toma entre 10 y 15 minutos y todo lo demás depende de
   ...
   corren sin configuración son /estudiante-derecho-co:entrevista-inicial y cualquier invocación con
...
4. En la primera ejecución después de una actualización, si existe un CLAUDE.md
   ...
   (~/.claude/plugins/cache/claude-legal-colombia/estudiante-derecho-co/<versión>/CLAUDE.md) pero no en la ruta de
...
|---|---|---|
| Gestor documental | [PENDIENTE ✓/✗] | El estudiante adjunta las sentencias y lecturas |
| Calendario | [PENDIENTE ✓/✗] | El plan de estudio se entrega en tabla |

*Revisar de nuevo: `/estudiante-derecho-co:entrevista-inicial --revisar-integraciones`*
...
- Consejo de Estado
- Universidad y programa académico
...
`~/.claude/plugins/config/claude-legal-colombia/estudiante-derecho-co/bitacora-verificacion.md` en una línea:
...
en `~/.claude/plugins/config/claude-legal-colombia/estudiante-derecho-co/asuntos/<slug>/` con su `asunto.md`, y las skills escriben ahí.

`Contexto entre asuntos:` off *(por defecto)* — no se leen archivos de otro asunto.

Ver `/estudiante-derecho-co:espacio-de-asunto`.
