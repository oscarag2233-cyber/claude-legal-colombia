...
2. Si ese archivo no existe o todavía tiene marcadores [PENDIENTE], DETENERSE antes
   ...
   /regulatorio-co:entrevista-inicial — toma entre 10 y 15 minutos y todo lo demás depende de
   ...
   corren sin configuración son /regulatorio-co:entrevista-inicial y cualquier invocación con
...
4. En la primera ejecución después de una actualización, si existe un CLAUDE.md
   ...
   (~/.claude/plugins/cache/claude-legal-colombia/regulatorio-co/<versión>/CLAUDE.md) pero no en la ruta de
...
# Perfil de práctica — Regulatorio y Vigilancia Normativa
...
|---|---|---|
| Correo y calendario | [PENDIENTE ✓/✗] | El boletín se entrega en línea |
| Canal de equipo | [PENDIENTE ✓/✗] | El digest se publica manualmente |
| Gestor documental | [PENDIENTE ✓/✗] | El mapa de obligaciones se guarda por fuera |

*Revisar de nuevo: `/regulatorio-co:entrevista-inicial --revisar-integraciones`*
...
- Imprenta Nacional — Diario Oficial
- Superintendencias sectoriales
- Ministerios y unidades administrativas especiales
...
#### Fuentes y periodicidad de la vigilancia
**Posición:** [PENDIENTE]
...
`~/.claude/plugins/config/claude-legal-colombia/regulatorio-co/bitacora-verificacion.md` en una línea:
...
en `~/.claude/plugins/config/claude-legal-colombia/regulatorio-co/asuntos/<slug>/` con su `asunto.md`, y las skills escriben ahí.

`Contexto entre asuntos:` off *(por defecto)* — no se leen archivos de otro asunto.

Ver `/regulatorio-co:espacio-de-asunto`.
