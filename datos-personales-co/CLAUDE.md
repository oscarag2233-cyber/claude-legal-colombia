...
2. Si ese archivo no existe o todavía tiene marcadores [PENDIENTE], DETENERSE antes
   ...
   /datos-personales-co:entrevista-inicial — toma entre 10 y 15 minutos y todo lo demás depende de
   ...
   corren sin configuración son /datos-personales-co:entrevista-inicial y cualquier invocación con
...
4. En la primera ejecución después de una actualización, si existe un CLAUDE.md
   ...
   (~/.claude/plugins/cache/claude-legal-colombia/datos-personales-co/<versión>/CLAUDE.md) pero no en la ruta de
...
# Perfil de práctica — Datos Personales y Habeas Data
...
|---|---|---|
| Gestor documental | [PENDIENTE ✓/✗] | El usuario adjunta políticas, contratos y evidencias |
| Correo | [PENDIENTE ✓/✗] | Las respuestas a titulares se entregan como borrador |
| Herramienta de anonimización | [PENDIENTE ✓/✗] | Se pide al usuario anonimizar antes de compartir documentos con datos |

*Revisar de nuevo: `/datos-personales-co:entrevista-inicial --revisar-integraciones`*
...
`~/.claude/plugins/config/claude-legal-colombia/datos-personales-co/bitacora-verificacion.md` en una línea:
...
en `~/.claude/plugins/config/claude-legal-colombia/datos-personales-co/asuntos/<slug>/` con su `asunto.md`, y las skills escriben ahí.

`Contexto entre asuntos:` off *(por defecto)* — no se leen archivos de otro asunto.

Ver `/datos-personales-co:espacio-de-asunto`.
