...
2. Si ese archivo no existe o todavía tiene marcadores [PENDIENTE], DETENERSE antes
   ...
   /tributario-co:entrevista-inicial — toma entre 10 y 15 minutos y todo lo demás depende de
   ...
   corren sin configuración son /tributario-co:entrevista-inicial y cualquier invocación con
...
4. En la primera ejecución después de una actualización, si existe un CLAUDE.md
   ...
   (~/.claude/plugins/cache/claude-legal-colombia/tributario-co/<versión>/CLAUDE.md) pero no en la ruta de
...
|---|---|---|
| Gestor documental | [PENDIENTE ✓/✗] | El usuario adjunta declaraciones, actos y soportes |
| Sistema contable | [PENDIENTE ✓/✗] | El usuario aporta cifras y conciliaciones |

*Revisar de nuevo: `/tributario-co:entrevista-inicial --revisar-integraciones`*
...
#### Apetito de riesgo frente a posiciones fiscales agresivas
**Posición:** [PENDIENTE]
...
`~/.claude/plugins/config/claude-legal-colombia/tributario-co/bitacora-verificacion.md` en una línea:
...
en `~/.claude/plugins/config/claude-legal-colombia/tributario-co/asuntos/<slug>/` con su `asunto.md`, y las skills escriben ahí.

`Contexto entre asuntos:` off *(por defecto)* — no se leen archivos de otro asunto.

Ver `/tributario-co:espacio-de-asunto`.
