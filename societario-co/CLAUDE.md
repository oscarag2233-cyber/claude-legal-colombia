...
2. Si ese archivo no existe o todavía tiene marcadores [PENDIENTE], DETENERSE antes
   ...
   /societario-co:entrevista-inicial — toma entre 10 y 15 minutos y todo lo demás depende de
   ...
   corren sin configuración son /societario-co:entrevista-inicial y cualquier invocación con
...
4. En la primera ejecución después de una actualización, si existe un CLAUDE.md
   ...
   (~/.claude/plugins/cache/claude-legal-colombia/societario-co/<versión>/CLAUDE.md) pero no en la ruta de
...
|---|---|---|
| Gestor documental | [PENDIENTE ✓/✗] | El usuario adjunta estatutos, actas y certificados |
| Firma electrónica | [PENDIENTE ✓/✗] | Las actas se suscriben por fuera del plugin |
| Cámara de comercio en línea | [PENDIENTE ✓/✗] | El usuario aporta el certificado de existencia y representación |

*Revisar de nuevo: `/societario-co:entrevista-inicial --revisar-integraciones`*
...
- Superintendencia de Sociedades (supervisión y facultades jurisdiccionales)
- Cámaras de Comercio (registro mercantil)
- Tribunales de arbitramento (cláusulas compromisorias societarias)
...
#### Postura sobre acuerdos de accionistas y su oponibilidad
**Posición:** [PENDIENTE]
...
`~/.claude/plugins/config/claude-legal-colombia/societario-co/bitacora-verificacion.md` en una línea:
...
en `~/.claude/plugins/config/claude-legal-colombia/societario-co/asuntos/<slug>/` con su `asunto.md`, y las skills escriben ahí.

`Contexto entre asuntos:` off *(por defecto)* — no se leen archivos de otro asunto.

Ver `/societario-co:espacio-de-asunto`.
