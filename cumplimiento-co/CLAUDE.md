...
2. Si ese archivo no existe o todavía tiene marcadores [PENDIENTE], DETENERSE antes
   ...
   /cumplimiento-co:entrevista-inicial — toma entre 10 y 15 minutos y todo lo demás depende de
   ...
   corren sin configuración son /cumplimiento-co:entrevista-inicial y cualquier invocación con
...
4. En la primera ejecución después de una actualización, si existe un CLAUDE.md
   ...
   (~/.claude/plugins/cache/claude-legal-colombia/cumplimiento-co/<versión>/CLAUDE.md) pero no en la ruta de
...
# Perfil de práctica — Cumplimiento — SAGRILAFT, PTEE y Antisoborno
...
|---|---|---|
| Gestor documental | [PENDIENTE ✓/✗] | El usuario adjunta manuales, matrices y evidencias |
| Listas restrictivas y verificación de contrapartes | [PENDIENTE ✓/✗] | La verificación se hace por fuera y el usuario aporta el resultado |
| Canal de denuncias | [PENDIENTE ✓/✗] | Los casos se cargan manualmente |

*Revisar de nuevo: `/cumplimiento-co:entrevista-inicial --revisar-integraciones`*
...
- Superintendencias sectoriales con régimen propio
...
#### Umbral de riesgo para debida diligencia intensificada
**Posición:** [PENDIENTE]
...
`~/.claude/plugins/config/claude-legal-colombia/cumplimiento-co/bitacora-verificacion.md` en una línea:
...
en `~/.claude/plugins/config/claude-legal-colombia/cumplimiento-co/asuntos/<slug>/` con su `asunto.md`, y las skills escriben ahí.

`Contexto entre asuntos:` off *(por defecto)* — no se leen archivos de otro asunto.

Ver `/cumplimiento-co:espacio-de-asunto`.
