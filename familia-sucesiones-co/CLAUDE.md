...
2. Si ese archivo no existe o todavía tiene marcadores [PENDIENTE], DETENERSE antes
   ...
   /familia-sucesiones-co:entrevista-inicial — toma entre 10 y 15 minutos y todo lo demás depende de
   ...
   corren sin configuración son /familia-sucesiones-co:entrevista-inicial y cualquier invocación con
...
4. En la primera ejecución después de una actualización, si existe un CLAUDE.md
   ...
   (~/.claude/plugins/cache/claude-legal-colombia/familia-sucesiones-co/<versión>/CLAUDE.md) pero no en la ruta de
...
# Perfil de práctica — Familia y Sucesiones
...
|---|---|---|
| Gestor documental | [PENDIENTE ✓/✗] | El usuario adjunta registros civiles y pruebas en cada consulta |
| Calendario | [PENDIENTE ✓/✗] | Los términos y audiencias se entregan en tabla, sin agendar |

*Revisar de nuevo: `/familia-sucesiones-co:entrevista-inicial --revisar-integraciones`*
...
- Notarías (sucesión y divorcio de mutuo acuerdo)
...
#### Cuándo se recomienda la vía notarial frente a la judicial
**Posición:** [PENDIENTE]
...
`~/.claude/plugins/config/claude-legal-colombia/familia-sucesiones-co/bitacora-verificacion.md` en una línea:
...
en `~/.claude/plugins/config/claude-legal-colombia/familia-sucesiones-co/asuntos/<slug>/` con su `asunto.md`, y las skills escriben ahí.

`Contexto entre asuntos:` off *(por defecto)* — no se leen archivos de otro asunto.

Ver `/familia-sucesiones-co:espacio-de-asunto`.
