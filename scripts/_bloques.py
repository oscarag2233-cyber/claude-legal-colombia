# -*- coding: utf-8 -*-
"""Bloques de texto compartidos por todos los plugins."""

MARCA = "claude-legal-colombia"
CFG = "~/.claude/plugins/config/" + MARCA
...
2. Si ese archivo no existe o todavía tiene marcadores [PENDIENTE], DETENERSE antes
   ...
   /{SLUG}:entrevista-inicial — toma entre 10 y 15 minutos y todo lo demás depende de
   ...
   corren sin configuración son /{SLUG}:entrevista-inicial y cualquier invocación con
...
4. En la primera ejecución después de una actualización, si existe un CLAUDE.md
   ...
   (~/.claude/plugins/cache/{MARCA}/{SLUG}/<versión>/CLAUDE.md) pero no en la ruta de
...
{CFG}/perfil-organizacion.md — un nivel arriba de este archivo, compartido por todos
...
GUARDARRAILES = """## Guardarraíles compartidos
...
`{CFG}/{SLUG}/bitacora-verificacion.md` en una línea:
...
ANDAMIAJE = """## Andamiaje, no anteojeras
...
CIERRE_COMUN = """## Preguntas sueltas del área
...
en `{CFG}/{SLUG}/asuntos/<slug>/` con su `asunto.md`, y las skills escriben ahí.

`Contexto entre asuntos:` off *(por defecto)* — no se leen archivos de otro asunto.

Ver `/{SLUG}:espacio-de-asunto`.
...
POSTURA = """## Postura ante decisiones jurídicas discutibles
