...
2. Si ese archivo no existe o todavía tiene marcadores [PENDIENTE], DETENERSE antes
   ...
   /contratos-comercial-co:entrevista-inicial — toma entre 10 y 15 minutos y todo lo demás depende de
   ...
   corren sin configuración son /contratos-comercial-co:entrevista-inicial y cualquier invocación con
...
4. En la primera ejecución después de una actualización, si existe un CLAUDE.md
   ...
   (~/.claude/plugins/cache/claude-legal-colombia/contratos-comercial-co/<versión>/CLAUDE.md) pero no en la ruta de
...
# Perfil de práctica — Contratos y Derecho Comercial
...
|---|---|---|
| Gestor documental (Drive / Box / SharePoint) | [PENDIENTE ✓/✗] | El usuario adjunta el contrato en cada revisión |
| Firma electrónica | [PENDIENTE ✓/✗] | El usuario tramita la firma por fuera del plugin |
| Gestor de contratos o carpeta de minutas | [PENDIENTE ✓/✗] | Se lleva un registro local de vencimientos y renovaciones |
| Correo y calendario | [PENDIENTE ✓/✗] | Las alertas se entregan en línea en lugar de agendarse |

*Revisar de nuevo: `/contratos-comercial-co:entrevista-inicial --revisar-integraciones`*
...
- Superintendencia de Industria y Comercio (consumidor y competencia)
...
### Posiciones de la casa por cláusula
...
`~/.claude/plugins/config/claude-legal-colombia/contratos-comercial-co/bitacora-verificacion.md` en una línea:
...
en `~/.claude/plugins/config/claude-legal-colombia/contratos-comercial-co/asuntos/<slug>/` con su `asunto.md`, y las skills escriben ahí.

`Contexto entre asuntos:` off *(por defecto)* — no se leen archivos de otro asunto.

Ver `/contratos-comercial-co:espacio-de-asunto`.
...
## Preferencias de revisión contractual
