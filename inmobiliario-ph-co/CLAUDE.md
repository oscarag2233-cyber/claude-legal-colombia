...
2. Si ese archivo no existe o todavía tiene marcadores [PENDIENTE], DETENERSE antes
   ...
   /inmobiliario-ph-co:entrevista-inicial — toma entre 10 y 15 minutos y todo lo demás depende de
   ...
   corren sin configuración son /inmobiliario-ph-co:entrevista-inicial y cualquier invocación con
...
4. En la primera ejecución después de una actualización, si existe un CLAUDE.md
   ...
   (~/.claude/plugins/cache/claude-legal-colombia/inmobiliario-ph-co/<versión>/CLAUDE.md) pero no en la ruta de
...
# Perfil de práctica — Inmobiliario y Propiedad Horizontal
...
|---|---|---|
| Gestor documental | [PENDIENTE ✓/✗] | El usuario adjunta folio de matrícula, escrituras y reglamento en cada análisis |
| Consulta registral en línea | [PENDIENTE ✓/✗] | El estudio se hace sobre el folio que aporte el usuario |

*Revisar de nuevo: `/inmobiliario-ph-co:entrevista-inicial --revisar-integraciones`*
...
- Curadurías urbanas y secretarías de planeación
- Jueces civiles municipales (restitución) y del circuito
- Alcaldías e inspecciones de policía (Ley 1801 de 2016)
...
#### Alcance por defecto del estudio de títulos (número de años de tradición)
**Posición:** [PENDIENTE]
...
`~/.claude/plugins/config/claude-legal-colombia/inmobiliario-ph-co/bitacora-verificacion.md` en una línea:
...
en `~/.claude/plugins/config/claude-legal-colombia/inmobiliario-ph-co/asuntos/<slug>/` con su `asunto.md`, y las skills escriben ahí.

`Contexto entre asuntos:` off *(por defecto)* — no se leen archivos de otro asunto.

Ver `/inmobiliario-ph-co:espacio-de-asunto`.
