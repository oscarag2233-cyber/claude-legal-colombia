...
2. Si ese archivo no existe o todavía tiene marcadores [PENDIENTE], DETENERSE antes
   ...
   /hub-constructor-legal-co:entrevista-inicial — toma entre 10 y 15 minutos y todo lo demás depende de
   ...
   corren sin configuración son /hub-constructor-legal-co:entrevista-inicial y cualquier invocación con
...
4. En la primera ejecución después de una actualización, si existe un CLAUDE.md
   ...
   (~/.claude/plugins/cache/claude-legal-colombia/hub-constructor-legal-co/<versión>/CLAUDE.md) pero no en la ruta de
...
# Perfil de práctica — Hub de Constructores Legales
...
|---|---|---|
| Repositorios de skills | [PENDIENTE ✓/✗] | El usuario aporta el enlace del repositorio a evaluar |
| Gestor documental | [PENDIENTE ✓/✗] | El registro de skills instaladas se guarda por fuera |

*Revisar de nuevo: `/hub-constructor-legal-co:entrevista-inicial --revisar-integraciones`*
...
- No aplica — control interno del entorno
...
`~/.claude/plugins/config/claude-legal-colombia/hub-constructor-legal-co/bitacora-verificacion.md` en una línea:
...
en `~/.claude/plugins/config/claude-legal-colombia/hub-constructor-legal-co/asuntos/<slug>/` con su `asunto.md`, y las skills escriben ahí.

`Contexto entre asuntos:` off *(por defecto)* — no se leen archivos de otro asunto.

Ver `/hub-constructor-legal-co:espacio-de-asunto`.
