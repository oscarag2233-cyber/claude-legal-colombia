...
2. Si ese archivo no existe o todavía tiene marcadores [PENDIENTE], DETENERSE antes
   ...
   /contencioso-administrativo-co:entrevista-inicial — toma entre 10 y 15 minutos y todo lo demás depende de
   ...
   corren sin configuración son /contencioso-administrativo-co:entrevista-inicial y cualquier invocación con
...
4. En la primera ejecución después de una actualización, si existe un CLAUDE.md
   ...
   (~/.claude/plugins/cache/claude-legal-colombia/contencioso-administrativo-co/<versión>/CLAUDE.md) pero no en la ruta de
...
# Perfil de práctica — Contencioso Administrativo y Contratación Estatal
...
|---|---|---|
| SECOP II | [PENDIENTE ✓/✗] | El usuario aporta el enlace o los documentos del proceso |
| Gestor documental | [PENDIENTE ✓/✗] | El usuario adjunta el acto administrativo y los antecedentes |
| Correo | [PENDIENTE ✓/✗] | Las peticiones se entregan como borrador |

*Revisar de nuevo: `/contencioso-administrativo-co:entrevista-inicial --revisar-integraciones`*
...
#### Postura sobre conciliación prejudicial y comité de conciliación
**Posición:** [PENDIENTE]
...
`~/.claude/plugins/config/claude-legal-colombia/contencioso-administrativo-co/bitacora-verificacion.md` en una línea:
...
en `~/.claude/plugins/config/claude-legal-colombia/contencioso-administrativo-co/asuntos/<slug>/` con su `asunto.md`, y las skills escriben ahí.

`Contexto entre asuntos:` off *(por defecto)* — no se leen archivos de otro asunto.

Ver `/contencioso-administrativo-co:espacio-de-asunto`.
